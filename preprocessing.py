...
# Train and evaluate models, including the ensemble model
trained_models = {}
model_r2_scores = {}  # To store R² scores
model_mae = {}        # To store MAE scores
model_rmse = {}       # To store RMSE scores
...
for name, model in models.items():
    ...
    model_r2_scores[name] = r2  # Store the R² score
    model_mae[name] = mae       # Store the MAE score
    model_rmse[name] = rmse     # Store the RMSE score
    
    print(f'{name} - MSE: {mse}, R²: {r2}, MAE: {mae}, RMSE: {rmse}')
    trained_models[name] = pipeline

# Now train the ensemble model using the individual models
...
# Save models and their metrics using joblib
for name, model in trained_models.items():
    joblib.dump(model, f'{name}_model.joblib')

joblib.dump(ensemble_model, 'Ensemble_model.joblib')

# Save the R² scores, MAE, and RMSE scores
